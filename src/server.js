var http = require('http'), https = require('https'), fs = require('fs'), path = require('path'), crypto = require('crypto'), nodeurl = require('url'), qs = require('qs'), utils = require('util'), serveStatic = require('./serveStatic'), slackr_utils = require('./slackr_utils'), db = require('./util/dbModule'), async = require('async'), app = http.createServer(handler), User = require('./objects/user').User, Team = require('./objects/team').Team;
...
function createUser(req, res, userdata) {
  slackr_utils.checkdata(req, res, [
    'uname',
    'upass',
    'uemail',
    'invite'
        ...
        self.create(userdata, function (args) {
          if (args.err) {
            ...
            res.end(JSON.stringify({
              ...
              url: self.xoauth_request_auth_url
            }));
          }
        });
      }
...
function getUserData(req, res, data) {
  slackr_utils.checkdata(req, res, [
    ...
    new User({ uname: data.uname }, function () {
      this.validateSession(data.session, function () {
