var http = require('http'), https = require('https'), slackr_utils = require('./slackr_utils'), fs = require('fs'), querystring = require('querystring'), crypto = require('crypto'), appMonitor = require('./appMonitor'), utils = require('util'), OAuth = require('mashape-oauth').OAuth, db = require('./util/dbModule');
...
module.exports.getToken = function (cb) {
  oa.getOAuthRequestToken(function (error, oauth_token, oauth_token_secret, results) {
    if (error) {
      ...
      cb(null, oauth_token, oauth_token_secret, results.xoauth_request_auth_url);  // Usually a redirect happens here to the /oauth/authorize stage
    }
  });
};
...
module.exports.refreshToken = function (oauth_token, oauth_secret, handle, cb) {
  oa.getOAuthAccessToken({
    oauth_token: oauth_token,
    oauth_token_secret: oauth_secret,
    parameters: { oauth_session_handle: handle }
  }, function (error, token, secret, result) {
    if (error) {
